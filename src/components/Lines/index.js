...
function Lines(props) {
    ...
    const mostLeftPoint = Math.min(firstNodeRect.left + HALF_NODE_WIDTH, parentCenterX);
    const mostRightPoint = Math.max(lastNodeRect.right - HALF_NODE_WIDTH, parentCenterX);
    const lineWidth = mostRightPoint - mostLeftPoint;

    const connectorLeft = mostLeftPoint - levelRect.left;
    const siblingsOnTheRight = firstNodeRect.left + HALF_NODE_WIDTH - parentCenterX > 0;

    const LOW_LEVEL_GAP = TOTAL_LEVEL_GAP / (totalWeight + 1) * weight;
    const HIGH_LEVEL_GAP = TOTAL_LEVEL_GAP - LOW_LEVEL_GAP;

    // sibling lines
    const offset = (siblingsOnTheRight ? firstNodeRect.left - mostLeftPoint + HALF_NODE_WIDTH : 0) + LINE_OFFSET;
    const siblingsPaths = Array(nodeRefs.length).fill().reduce((acc, cur, i) => {
        return acc.concat(
            <path key={i} d={`M${offset + i * NODE_OUTER_WIDTH},${LOW_LEVEL_GAP} l00,${HIGH_LEVEL_GAP}`}/>
        );
    }, []);
    ...
    // add parent line
    const parentLine = `M${(siblingsOnTheRight ? 0 : parentCenterX - mostLeftPoint) + LINE_OFFSET},00 l00,${LOW_LEVEL_GAP}`;

    return (
        <svg data-parentId={parentId} style={{ left: connectorLeft }} class="lines" width={lineWidth + LINE_OFFSET * 2} height={TOTAL_LEVEL_GAP}>
            {siblingsPaths}
